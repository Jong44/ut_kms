    ...
    return (
        <div className="shadow-[0px_2px_16px_0px_#d3d3d3] rounded-2xl">
            <div className="bg-primary rounded-t-2xl flex items-center justify-between px-5 py-4">
                ...
                <input type="text" placeholder={placeholderSearch} className="bg-[#F5F5F5] text-sm px-4 py-2 rounded-xl w-[40%] " onChange={handleSearch}  />
            </div>
            <div>
                    ...
                    <thead className=" text-primary text-left">
                        <tr>
                            {headerData.map((header, index) => (
                                <th key={index} className={`py-3 px-5 ${header.width ? `w-[${header.width}]` : ''} ${header.textAlign ? header.textAlign : ''} ${header.isBold ? 'font-bold' : ''}`}>{header.label}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {filteredData.length > 0 ? filteredData.map((body, index) => (
                            <tr key={index} onClick={body.onClick} className="border-b border-[#bfbfbf] hover:bg-[#F5F5F5] cursor-pointer" >
                                {body.data.map((data, index) => (
                                    <td key={index} className={`py-3 px-5 ${data.width ? `w-[${data.width}]` : ''} ${data.textAlign ? data.textAlign : ''} ${data.isBold ? 'font-bold' : ''}`}>{data.data}</td>
                                ))}
                            </tr>
                        )) : (
                            <tr>
